...
def deepnn(x):
  ...
  x_image = tf.reshape(x, [-1, 480, 240, 1])
    ...
    with tf.Session() as sess:
      ...
      for i in range(20000):
        ...
        if i % 300 == 0 and i != 0:
          ...
          test_accuracy(dataset, accuracy, x, y_, keep_prob)
        train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})

def test_accuracy(dataset, accuracy_tf_step, x, y_, keep_prob):
  batch_size = 10
  batch_num = int(dataset.test.num_examples / batch_size)
  test_accuracy = 0
      
  for i in range(batch_num):
      batch = dataset.test.next_batch(batch_size, False)
      test_accuracy += accuracy_tf_step.eval(feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
