...
def load_trains_dataset(with_noise=True):
  ...
  with open('./image_labels_dict', 'rb') as image_labels_file:
    image_labels_dict = pickle.load(image_labels_file)
  ...
  for index, image_file in enumerate(img_filenames):
    ...
    image_matrix = numpy.reshape(image_matrix, (IMAGE_WIDTH*IMAGE_HEIGHT))
    image_matrix = image_matrix.astype(numpy.float32)
    image_matrix = numpy.multiply(image_matrix, 1.0 / 255.0) # Convert from [0, 255] -> [0.0, 1.0]
